...
if [[ $javaversion = *"64-Bit"* ]]; then
	...
	java -classpath "`dirname "$0"`/bin" -Xms16m -Xmx4096m com.asofterspace.picturizer.Picturizer "$@"
else
	# we are running 32 bit - oh no! assign only a tinly little bit of heap!
	java -classpath "`dirname "$0"`/bin" -Xms16m -Xmx1024m com.asofterspace.picturizer.Picturizer "$@"
fi
